...
import { Check, X, Eye, Trash2, Users, Package, ArrowUpDown } from "lucide-react"
...
export default function AdminPage() {
  ...
  // For demo, set loading to false immediately
  useEffect(() => {
    setLoading(false)
  }, [])
          ...
          <div className="space-y-4">
            {items.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-neutral-900 mb-2">No {activeTab} items</h3>
                <p className="text-neutral-600">All items have been processed</p>
              </div>
            ) : (
              items.map((item) => (
                <div key={item.id} className="card">
                  <div className="flex items-start space-x-6">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-neutral-600 mb-2 line-clamp-2">{item.description}</p>
                      <div className="flex items-center space-x-4 text-sm text-neutral-500">
                        <span>by @{item.profiles?.username}</span>
                        <span>{item.category}</span>
                        <span>{item.condition}</span>
                        <span>{item.points_value} pts</span>
                      </div>
                    </div>
                    <div className="flex space-x-2">
                      <button
                        onClick={() => router.push(`/items/${item.id}`)}
                        className="p-2 text-neutral-600 hover:text-primary-600 border border-neutral-300 rounded-lg"
                      >
                ...
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id} className="border-b border-neutral-100">
                      ...
                      <td className="py-3 px-4 text-neutral-600">{user.points}</td>
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${
                            user.is_admin ? "bg-primary-100 text-primary-800" : "bg-neutral-100 text-neutral-800"
                          }`}
                        >
                          {user.is_admin ? "Admin" : "User"}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-neutral-600">{new Date(user.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="space-y-4">
            {swaps.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-neutral-900 mb-2">No swaps yet</h3>
                <p className="text-neutral-600">Swaps will appear here once users start trading</p>
              </div>
            ) : (
              swaps.map((swap) => (
                <div key={swap.id} className="card">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-neutral-900 mb-1">{swap.items?.title}</h3>
                      <p className="text-neutral-600 text-sm">
                        @{swap.profiles?.username} → @{swap.profiles?.username}
                      </p>
                      <p className="text-neutral-500 text-sm">{new Date(swap.created_at).toLocaleDateString()}</p>
                    </div>
                    <span
